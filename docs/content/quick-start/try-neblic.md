...
1. To configure the sampler, you will use the `neblictl` command. First, you can download it from the [releases page](https://github.com/neblic/platform/releases) or install it using the `Go` install command:
    ``` shell
    go install github.com/neblic/platform/cmd/neblictl@latest
    ```
    Once installed, you can connect to Neblic's control server just by executing it since, by default, it connects to a server exposed at `localhost`.
